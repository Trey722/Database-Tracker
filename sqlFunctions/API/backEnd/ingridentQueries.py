...
def addINGREDIENTS(INGREDIENTS_ID, INGREDIENTS_NAME, AMOUNT_LEFT, MIN_AMOUNT, MAX_AMOUNT, COST):
    query = f"INSERT INTO INGREDIENTS(INGREDIENT_ID, INGREDIENT_NAME, AMOUNT_LEFT, MIN_AMOUNT, MAX_AMOUNT, COST) VALUES ({INGREDIENTS_ID}, {INGREDIENTS_NAME}, {AMOUNT_LEFT}, {MIN_AMOUNT}, {MAX_AMOUNT}, {COST})"

    try:
        return start.executeQuery(query)
    except Exception as e:
        return [False, str(e)]
    
    
    
def searchIngridents(INGREDIENTS_ID):
    try:
        INGREDIENTS_ID = int(INGREDIENTS_ID)
        query = f"SELECT * FROM INGREDIENTS WHERE INGREDIENT_ID = {INGREDIENTS_ID}"
    except:
